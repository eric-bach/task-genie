...
export const lambdaHandler = async (event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    try {
      ...
      logger.info('✅ Bedrock Agent Runtime completed', { sessionId, textResponse });
    } catch (agentError) {
      logger.error('🛑 Bedrock Agent Runtime failed', { sessionId, error: String(agentError) });
    }
    ...
    if (error instanceof InvalidWorkItemError) {
      logger.error(`🛑 Validation error: ${error.message}`, { details: error, statusCode: error.code });
      return {
