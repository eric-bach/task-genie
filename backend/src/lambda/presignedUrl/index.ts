...
export const handler = async (event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    if (!area) {
      logger.error('area parameter is required');
      return {
        statusCode: 400,
        headers: {
    ...
    const pathComponents = [area, businessUnit, system].filter(Boolean);
    const key = `${pathComponents.join('/')}/${fileName}`;

    logger.info(`Generating presigned URL for bucket: ${bucketName}, key: ${key}`);
