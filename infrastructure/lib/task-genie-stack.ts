...
export class TaskGenieStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const resultsTable = new Table(this, 'TaskGenieResults', {
      tableName: `${APP_NAME}-results`,
      partitionKey: {
        name: 'executionName',
    ...
    const pollExecutionFunction = new NodejsFunction(this, 'PollExecution', {
      runtime: Runtime.NODEJS_22_X,
      functionName: `${APP_NAME}-poll-execution`,
      handler: 'handler',
    ...
    new LogRetention(this, 'PollExecutionLogRetention', {
      logGroupName: `/aws/lambda/${pollExecutionFunction.functionName}`,
      retention: RetentionDays.ONE_MONTH,
    });
  ...
  "stateMachineArn": "${stateMachine.stateMachineArn}"
    ...
    const errorLogs = new LogQueryWidget({
      ...
      logGroupNames: [
        ...
        `/aws/lambda/${pollExecutionFunction.functionName}`,
      ],
      queryString: `SOURCE '/aws/lambda/${evaluateUserStoryFunction.functionName}' | SOURCE '/aws/lambda/${defineTasksFunction.functionName}' | SOURCE '/aws/lambda/${createTasksFunction.functionName}' | SOURCE '/aws/lambda/${addCommentFunction.functionName}' | SOURCE '/aws/lambda/${sendResponseFunction.functionName}' | SOURCE '/aws/lambda/${pollExecutionFunction.functionName}' 
